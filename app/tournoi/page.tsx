            ...
            {tournois.map((t) => (
              <tr key={t.id} className="hover:bg-[#393c4d] text-white">
                ...
                <td className="p-2 border space-x-1">
                  ...
                  <button
                    onClick={() => {
                      // Example: Download tournament info as JSON
                      const tournoiData = {
                        id: t.id,
                        name: t.name,
                        city: t.city,
                      ...
                      const blob = new Blob([JSON.stringify(tournoiData, null, 2)], { type: 'application/json' });
                      const url = URL.createObjectURL(blob);
                      const a = document.createElement('a');
                      a.href = url;
                      a.download = `tournoi_${t.id}.json`;
                      a.click();
                      URL.revokeObjectURL(url);
                    }}
                    className="px-2 py-1 bg-green-600 text-white rounded hover:bg-green-700 border border-black"
                  >
                    Télécharger
                  </button>
                </td>
