...
export default function TournamentDetailsPage() {
  ...
  const userId = 1; // Remplace par le vrai userId (session)
  ...
  useEffect(() => {
    if (!id) return; // attend que le param soit dispo
    ...
    const fetchMatches = async () => {
      const res = await fetch(`/api/match/by-tournament/${id}`);
      if (res.ok) {
        const data = await res.json();
        setMatchesExist(data.length > 0);
      }
    };
  ...
  const supprimerCategorie = async (catId: number) => {
  ...
  const handlePreviewAllCompetitors = () => {
    // Pour chaque catégorie, filtre les compétiteurs correspondants (par rank, genre, <=5 ans ?)
            ...
            <table className="w-full border text-sm bg-white border-black">
              <thead>
                <tr>
                  ...
                  <th className="border px-2 py-1 text-black">Anniversaire</th>
                  <th className="border px-2 py-1 text-black">Age</th>
                  <th className="border px-2 py-1 text-black">Rang</th>
  ...
  <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
    <div className="bg-white p-6 rounded shadow-md max-w-md w-full">
      <h2 className="text-xl font-bold mb-4 text-black">Créer une Catégorie</h2>
      <form onSubmit={handleCreateCategory} className="space-y-3">
        <div>
          <label className="block mb-1 font-semibold text-black">Nom</label>
          <input
        ...
        <div>
          <label className="block mb-1 font-semibold text-black">Rang</label>
          <select
            className="border px-2 py-1 rounded w-full text-black"
        ...
        <div>
          <label className="block mb-1 font-semibold text-black">Genre</label>
          <select
            className="border px-2 py-1 rounded w-full text-black"
        ...
        <div>
          <label className="block mb-1 font-semibold text-black">Catégorie de poids</label>
          <select
            className="border px-2 py-1 rounded w-full text-black"
        ...
        <div>
          <label className="block mb-1 font-semibold text-black">Groupe d'âge</label>
          <select
            className="border px-2 py-1 rounded w-full text-black"
